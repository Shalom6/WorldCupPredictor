/**
 * Generic Sofascore fetch — injected into browser via puppeteer.
 */
export async function fetchTeamFromSofascore(config, opts = {}) {
  const TEAM_ID = config.sofascoreId;
  const TEAM_NAME = config.name;
  const SOFASCORE_NAMES = [TEAM_NAME, ...(config.aliases ?? [])].filter(Boolean);
  ...
  teamMatchResults.sort((a, b) => b.date.localeCompare(a.date));
  ...
  return {
    ...
    teamMatchResults: teamMatchResults.slice(0, 15),
    officialSquad,
    players
  };
}
