/**
 * Fetch South Africa stats from Sofascore via headless browser.
 *   node scripts/run-sofascore-south-africa-fetch.mjs
 */
import fs from 'fs';
...
const fetchModulePath = path.join(__dirname, 'sofascore-south-africa-fetch.js');

const fnBody = fs
  .readFileSync(fetchModulePath, 'utf8')
  .replace(/^\/\*\*[\s\S]*?\*\/\s*/, '')
  .replace(/^export async function fetchSouthAfricaFromSofascore/, 'async function fetchSouthAfricaFromSofascore');

const browserScript = `${fnBody}
return fetchSouthAfricaFromSofascore({ maxEvents: 22, delayMs: 280 });
`;
