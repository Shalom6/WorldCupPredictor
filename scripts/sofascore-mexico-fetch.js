/**
 * Sofascore fetch logic — run inside browser context (see import-sofascore-mexico.mjs).
 * Mexico team id: 4781
 */
export async function fetchMexicoFromSofascore(opts = {}) {
  const MEXICO_TEAM_ID = 4781;
  ...
  for (const p of pages) {
    const payload = await fetch(
      `https://api.sofascore.com/api/v1/team/${MEXICO_TEAM_ID}/events/last/${p}`
    ).then((r) => r.json());
  ...
  events = events.filter(
    (e) =>
      ...
      (e.homeTeam?.id === MEXICO_TEAM_ID ||
        e.awayTeam?.id === MEXICO_TEAM_ID ||
        e.homeTeam?.name === 'Mexico' ||
        e.awayTeam?.name === 'Mexico')
  );
  ...
  for (const ev of events) {
    ...
    const mexicoIsHome =
      ev.homeTeam?.id === MEXICO_TEAM_ID || ev.homeTeam?.name === 'Mexico';
    const side = mexicoIsHome ? lineups.home : lineups.away;
    if (!side?.players?.length) continue;

    const date = new Date(ev.startTimestamp * 1000).toISOString().slice(0, 10);
    const opponent = mexicoIsHome ? ev.awayTeam?.name : ev.homeTeam?.name;
    const hs = ev.homeScore?.current ?? ev.homeScore?.display ?? 0;
    const as = ev.awayScore?.current ?? ev.awayScore?.display ?? 0;
    const mexGoals = mexicoIsHome ? hs : as;
    const oppGoals = mexicoIsHome ? as : hs;
    let result = 'D';
    if (mexGoals > oppGoals) result = 'W';
    if (mexGoals < oppGoals) result = 'L';
  ...
  // Current squad from Sofascore (includes players with 0 logs)
  const squadPayload = await fetch(
    `https://api.sofascore.com/api/v1/team/${MEXICO_TEAM_ID}/players`
  ).then((r) => r.json());
