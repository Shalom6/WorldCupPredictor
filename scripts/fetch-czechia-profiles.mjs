/**
 * Fetch player profiles from Sofascore for Czechia squad.
 *   node scripts/fetch-czechia-profiles.mjs
 */
import fs from 'fs';
...
const squadPath = path.join(root, 'data', 'czechia.json');
const outPath = path.join(root, 'data', 'czechia-profiles.json');
    ...
    const profiles = await page.evaluate(async (playerIds) => {
      ...
      for (const id of playerIds) {
        ...
        try {
          ...
          const nt =
            (d.statistics ?? []).find(
              (s) => s.team?.name?.includes('Czech') || s.team?.name?.includes('Czechia')
            ) ?? (d.statistics ?? [])[0];
          out.push({
            sofascoreId: id,
