/**
 * Fetch player profiles from Sofascore for South Korea squad.
...
const squadPath = path.join(root, 'data', 'south-korea.json');
const outPath = path.join(root, 'data', 'south-korea-profiles.json');
      ...
      for (const id of playerIds) {
        ...
        try {
          ...
          const age = p.dateOfBirthTimestamp
            ...
            : null;
          const nt = (d.statistics ?? []).find((s) => s.team?.name?.includes('Korea')) ??
            (d.statistics ?? [])[0];
          out.push({
            sofascoreId: id,
